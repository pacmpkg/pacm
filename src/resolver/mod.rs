...
impl Resolver {
    ...
    pub fn pick_version(&self, versions: &BTreeMap<Version, String>, range: &str) -> Result<(Version, String)> {
    let norm = canonicalize_npm_range(range);
    let req = if norm == "*" { VersionReq::STAR } else { VersionReq::from_str(&norm).map_err(|e| anyhow!("invalid semver range '{}'(orig '{}'): {}", norm, range, e))? };
        let mut candidates: Vec<_> = versions.iter().collect();
        candidates.sort_by(|a, b| b.0.cmp(a.0)); // descending
        for (ver, tarball) in candidates {
            if req.matches(ver) { return Ok((ver.clone(), tarball.clone())); }
        }
...
pub fn map_versions(meta: &crate::fetch::NpmMetadata) -> BTreeMap<Version, String> {
    ...
    for (_k, v) in &meta.versions { if let Ok(ver) = Version::parse(&v.version) { map.insert(ver, v.dist.tarball.clone()); } }
    map
}

/// Normalize common npm range syntaxes into a form the Rust semver crate accepts.
/// This is a pragmatic, partial implementation – it handles spaced comparators,
/// hyphen ranges, wildcards like 1.x / 1.2.x, and bare majors or major.minor.
fn canonicalize_npm_range(input: &str) -> String {
    let s = input.trim();
    if s.is_empty() || s == "*" || s == "latest" { return "*".into(); }

    // Hyphen range: "1.2.3 - 2.3.4" => ">=1.2.3, <=2.3.4"
    if let Some(idx) = s.find(" - ") { // require spaces around - to avoid confusion with prerelease
    ...
    if tokens.len() > 1 {
        ...
        while i < tokens.len() {
            ...
            if is_op(t) {
                if let Some(ver) = next { comps.push(format!("{}{}", t, ver)); i += 2; continue; }
            ...
            if is_version_like(t) {
                ...
                if t.contains('x') || t.contains('X') || t.contains('*') { return expand_wildcard(t); }
                // Bare major / major.minor expansions handled below
                if is_numeric(t) { return format!("^{}.0.0", t); }
                if count_dots(t) == 1 && t.chars().all(|c| c.is_ascii_digit() || c == '.') { // major.minor
                    let (maj, min) = {
                        let mut parts = t.split('.');
                        (parts.next().unwrap(), parts.next().unwrap())
                    };
                    // >=maj.min.0 <maj.(min+1).0
                    if let Ok(min_i) = min.parse::<u64>() { return format!(">={maj}.{min}.0, <{maj}.{}.0", min_i+1); }
                }
                comps.push(format!("={}", t));
                i += 1;
                continue;
    ...
    if is_numeric(s) { return format!("^{}.0.0", s); }
...
fn is_op(t: &str) -> bool { matches!(t, ">"|"<"|">="|"<="|"="|"^"|"~") }
fn is_numeric(t: &str) -> bool { !t.is_empty() && t.chars().all(|c| c.is_ascii_digit()) }
fn count_dots(t: &str) -> usize { t.chars().filter(|&c| c=='.').count() }
fn is_version_like(t: &str) -> bool {
    let mut has_digit = false;
    for c in t.chars() {
        if c.is_ascii_digit() { has_digit = true; continue; }
        if !matches!(c, '.' | '-' | 'x' | 'X' | '*' | 'a'..='z' | 'A'..='Z') { return false; }
    }
    has_digit
}
...
fn expand_wildcard(pattern: &str) -> String {
    ...
    if parts.is_empty() { return pattern.to_string(); }
    if parts.len()==2 && (parts[1].eq_ignore_ascii_case("x") || parts[1]=="*") {
        if let Ok(maj) = parts[0].parse::<u64>() { return format!(">={maj}.0.0, <{} .0.0", maj+1).replace("  ", " ").replace(" .", "."); }
    }
    if parts.len()==3 && (parts[2].eq_ignore_ascii_case("x") || parts[2]=="*") {
        if let (Ok(maj), Ok(min)) = (parts[0].parse::<u64>(), parts[1].parse::<u64>()) { return format!(">={maj}.{min}.0, <{maj}.{}.0", min+1); }
    }
